...
class BasePDESolver1D(ABC):
    def __init__(self, ax: float, bx: float, nx: int, n_states: int, bc: str = 'periodic'):
        self.ax: float = ax
        self.bx: float = bx
        self.n_cells: int = nx
    ...
    def run_solver(self, u0: NDArray,
        ...
        u[self.n_ghost_cells:-self.n_ghost_cells, :, 0] = u0
    ...
    @staticmethod
    def minmod(forward_approx: float, backward_approx: float, central_approx: float):
    ...
    def du_dx(self, um1: float, u: float, up1: float, theta: float):
    ...
    def plot_solution(self, solution_data: List[SolutionData], n_times_stamps: int = 4, t_max: float = None):
    ...
    def plot_animation(self, solution_data, t_max, equation: str):
        ...
        def update(frame):
            ...
            for sol in solution_data:
                for j in range(self.n_states):
                    sol.plot_solution(j, frame, self.x)
